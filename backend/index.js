...
app.post("/adminproduct",upload.single("pro_pic"),async(req,res)=>{
    ...
    const rec=new productmodel({catid:catid,pname:pname,price:price,offerprice:offerprice,pic:pic,description:desc
    });
...
app.post("/signup",async(req,res)=>{
    ...
    const rec1=await usermodel.find({email:email});
    const rec2=await usermodel.find({mobile:mobile});
    if(rec1.length>0)
    {
        res.json({msg:"Email is already existing.."})
    }
    else if(rec2.length>0)
    {
        res.json({msg:"Mobile is already existing.."})
    }
    else{
